...
for b in ["ppl", "qhull", "palp"]:
    ...
    g = p.random_triangulations_fast(N=2, c=1)
    next(g)
    next(g)
    g = p.random_triangulations_fair(N=2)
    next(g)
    next(g)
...
for b in ["cgal", "qhull", "topcom"]:
    ...
    p.triangulate(backend=b, heights=[10,2,2,4,4,5,2,0,-2,-1,-2,-1,-2,-1,-1,1])
...
cy.clear_cache()
cy.toric_mori_cone(in_basis=False, from_intersection_numbers=True)
