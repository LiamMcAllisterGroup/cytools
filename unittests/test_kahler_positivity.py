...
class Kahler_Positivity_Test(unittest.TestCase):
    def setUp(self):
        with open(test_data_path,'r') as fp:
            ...
            self.test_data = self.test_data[:40] + self.test_data[40:-2:20] + self.test_data[-2:]
    ...
    def test_positivity_cgal(self):
        ...
        for doc in data_iter:
            if doc['N_favorable']=="True":
                ...
                options = ((p,pp) for p in ('qhull', 'cgal', 'topcom') for pp in ('all', 'sksparse', 'scipy'))
                for opt in options:
                    positivity_data = compute_positivity_data(poly, opt[0], opt[1])
                    self.assertTrue(positivity_data['CY_Volume']>0, 
                        msg="Negative CY Volume. Options:" + str(opt) +
                         ": \n" + str(positivity_data['CY_Volume']))
                    self.assertTrue(all(i>0 for i in positivity_data['Divisor_Volumes']),
                        msg="Negative divisor volumes. Options:" + str(opt) +
                         ": \n" + str(positivity_data['Divisor_Volumes']))
                    self.assertTrue(all(i>0 for i in positivity_data['Kahler_eigenvalues']),
                        msg="Negative Kahler metric eigenvalues. Options:" + str(opt) +
