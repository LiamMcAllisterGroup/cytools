...
test_data_path = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "data/cy_data.json"
)
    ...
    def test_intersection_numbers(self):
        ...
        for doc in data_iter:
            ...
            if sage_favorable:
                ...
                cy = frst.get_cy()
                cyt_int_nums_origin_raw = cy.intersection_numbers(
                    in_basis=False, zero_as_anticanonical=True
                )
                cyt_int_nums_raw = [
                    [ii[0] - 1, ii[1] - 1, ii[2] - 1] + [cyt_int_nums_origin_raw[ii]]
                    for ii in cyt_int_nums_origin_raw
                    if ii[0] != 0 and ii[1] != 0 and ii[2] != 0
                ]
                ...
                transform_dict_1 = {
                    sage_index[tuple(pp)]: cyt_index[tuple(pp)] for pp in cyt_points
                }
                sage_int_nums = sorted(
                    [
                        sorted(
                            [
                                transform_dict_1[ii[0]],
                                transform_dict_1[ii[1]],
                                transform_dict_1[ii[2]],
                            ]
                        )
                        + [ii[3]]
                        for ii in sage_int_nums_raw
                    ]
                )
                cyt_int_nums = sorted([ii for ii in cyt_int_nums_raw])
                self.assertTrue(
                    sage_int_nums == cyt_int_nums,
    ...
    def test_sr_ideal(self):
        ...
        for doc in data_iter:
            ...
            if sage_favorable:
                ...
                sage_sr_ideal = sorted(
                    [
                        sorted([transform_dict_1[ss] + 1 for ss in s])
                        for s in sage_sr_ideal_raw
                    ]
                )
                cyt_sr_ideal = sorted([sorted(s) for s in cyt_sr_ideal_raw])
                self.assertTrue(
                    sage_sr_ideal == cyt_sr_ideal,
    ...
    def test_second_chern_class(self):
        ...
        for doc in data_iter:
            ...
            if sage_favorable:
                ...
                cyt_c2 = cy.second_chern_class(
                    in_basis=False, include_origin=False
                ).tolist()
                sage_index = {tuple(pp): p for p, pp in enumerate(sage_points)}
                cyt_index = {tuple(pp): p for p, pp in enumerate(cyt_points)}
                transform_dict = {
                    cyt_index[tuple(pp)]: sage_index[tuple(pp)] for pp in cyt_points
                }
                sage_c2 = [
                    sage_c2_raw[transform_dict[c]] for c in range(len(sage_c2_raw))
                ]
                self.assertTrue(
                    sage_c2 == cyt_c2,
